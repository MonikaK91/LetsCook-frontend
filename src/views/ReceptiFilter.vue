...
export default {
  ...
  async mounted() {
    this.fetchRecipesFilter(); //poziv funkcije
  },
  methods: {
    //metoda za dohvat recepata po odabranom filteru
    async fetchRecipesFilter() {
      let filter = this.filter; //filter iz props dijela
      let cards = await Recepti.getAllFilter(filter);
      this.cards = cards;
    },
    //metoda za odjavu
    logout() {

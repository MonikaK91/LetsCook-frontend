<template>
  <div>
    ...
    <!-- tekst ispod se pojavljuje ako korisnik još nema vlastitih recepata -->
    <p v-if="this.cards == ''">Nema još oglasa</p>
    <!-- za prikaz recepata -->
...
export default {
  ...
  mounted() {
    this.fetchRecipesUsername(); //poziv funkcije
  },
  methods: {
    //metoda za dohvat recepata trenutnog korisnika
    async fetchRecipesUsername() {
      let user = this.username; //username je iz props dijela
      let cards = await Recepti.getAllUsername(user);
      this.cards = cards;
    },
    logout() {
